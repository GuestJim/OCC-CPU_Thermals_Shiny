...
observeEvent(list(input$FREQ.COEF, brushFREQzoom$x), {	
	req(brushFREQzoom$FILTER)
	hold	<-	lapply(unique(DATA$dataALL$Thread), function(i) {
		tagList(
			strong(paste0("Thread: ", i)),
			renderPlot({
				graphFREQfilt(DATA$dataALL$Thread == i, isolate(input$FREQ.COEF), brushFREQzoom$x)
				}, height = 480)
			)
		})
...
observeEvent(list(input$FREQ.COEF, brushPOWERzoom$x), {	
	req(brushPOWERzoom$FILTER)
	hold	<-	lapply(unique(DATA$dataALL$Core), function(i) {
		tagList(
			strong(paste0("Core: ", i)),
			renderPlot({
				graphPOWERfilt(DATA$dataALL$Core == i, isolate(input$FREQ.COEF), brushPOWERzoom$x)
				}, height = 480)
			)
		})
